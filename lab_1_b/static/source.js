document.addEventListener('DOMContentLoaded', function () {
	...
	function enableProtection() {
		if (!isProtected) {
			if (localStorage.getItem('protectionEnabled') !== 'true') alert('Защита включена')
	...
	async function disableProtection() {
		...
		if (userPasswordHash === passwordHash) {
			localStorage.setItem('protectionEnabled', 'false')
	...
	// Инициализация состояния защиты при загрузке
	function initializeProtectionStatus() {
		if (localStorage.getItem('protectionEnabled') === 'true') {
			enableProtection() // Активируем защиту без запроса пароля
		} else {
			// Если защита не включена, просто отключаем все методы защиты
			localStorage.setItem('protectionEnabled', 'false')
			document.body.classList.remove('protected')
			document.removeEventListener('contextmenu', disableContextMenu)
			document.removeEventListener('keydown', disableCtrlC)
